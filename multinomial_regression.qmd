# Multinomial Logistic Regression for Nominal Category Outcomes {#sec-multinom}

\index{multinomial logistic regression|(}
...
\index{data sets!health\_insurance@\texttt{health\_insurance}}The `health_insurance` data set consists of the following fields:
...
It looks like two of these columns should be converted to factor---`product` and `gender`---so let's do that and then run a pairplot for a quick overview of any patterns, which can be seen in @fig-insurance-pairplot.

::: {#fig-insurance-pairplot}
```{r} 
#| fig.align: "center"
library(GGally)
...
Pairplot of the `health_insurance` data set
:::
...
\index{multinomial logistic regression!coefficients}Notice that the output of `summary(multi_model)` is much less detailed than for our standard binomial models, and it effectively just delivers the coefficients and standard errors of the two models against the reference.  To determine whether specific input variables are significant we will need to calculate the p-values of the coefficients manually by calculating the z-statistics and converting (we covered this hypothesis testing methodology in @sec-means-sig).
...
\index{Pseudo-R-squared!in multinomial logistic regression}As with the binomial case, a variety of Pseudo-$R^2$ methods are available to assess the fit of a multinomial logistic regression model, although some of our previous variants (particularly Tjur) are not defined on models with more than two outcome categories.
...
\index{data sets!health\_insurance@\texttt{health\_insurance}}Use the same `health_insurance` data set from this chapter to answer these questions.

1.  Complete the full stratified approach to modeling the three product choices that was started in @sec-stratified.  Calculate the coefficients, odds ratios and p-values in each case.
...
4.  Verify that the coefficients for Product C against reference Product B matches those calculated in @sec-changing-ref.
5.  Carefully write down your interpretation of the odds ratios calculated in the previous question.
6.  Use the process described in @sec-elim to simplify the multinomial model in Question 3.


\index{regression!multinomial|see {multinomial logistic regression}}
