...
Recall from  @sec-mean-var-sd the definition of the population variance of $y$, notated as $\mathrm{Var}(y)$.  Note that it is defined as the average of the squares of the residuals around the mean of $y$. Therefore $\mathrm{Var}(y)$ represents the average squared residual error of a random model.  This calculates in this case to `{r} round((9/10)*var(d$Final), 2)`.  Let's overlay our fitted model onto this random model in @fig-model-overlay.
...
In this case, our inputs are a set of $p$ variables $x_1, x_2, \dots, x_p$.  Extending the linear equation in  @sec-linear-single, we seek to develop an equation of the form:
