...
* `Test`: The candidate's score out of 100 on a written test.
...
@fig-selection-ci shows the posterior distribution of the GPA coefficient.  The 95% credible interval includes zero, suggesting that after controlling for `Int`, `Pres` and `Test`, `GPA` does not have a significant effect on the hiring decision. This supports the conditional independence implied by our DAG.
...
posterior_summary(exp(as.matrix(model_causal)), probs = c(0.025, 0.975))["b_Test", c("Q2.5", "Q97.5")]
```

This suggests that, under the assumptions of our causal model, there is a 95% probability that a one-point increase in the test score increases the odds of being hired by between 0.6% and 4.2%.
