# Causal Inference - Moving From Association to Causation

This book is about statistics, and over many previous chapters we have covered numerous classical and modern approaches to explanatory statistical modeling.  Historically, statisticians have been careful to view such models as descriptive tools for summarizing associations in data, rather than as tools for making causal claims. In our previous sections on interpreting the output of models, our language has been very intentional in relation to this.  We have used phrases like 'a change in x *is associated with* a change in y', and we have deliberately avoided phrases like 'a change in x *causes* a change in y' or 'a change in x *leads to* a change in y'. This caution reflects a long-standing principle in statistics, and one which has become a very popular refrain over the years: correlation (or association) does not imply causation.

In reality, however, most of us who practice statistics do not employ our toolkit and skillset to simply describe associations.  We want to understand the dynamics of the problem we are interested in, and often that means we want to understand whether causal relationships exist.  For example, we may want to know if a new employee benefit has caused an increase in employee satisfaction, or if a change in the structure of an education program has caused better learning outcomes.  Moreover, when our leaders and clients want us to help them make decisions, they often have questions like *'Why is this happening?'* or *'What would happen if...?'*.  Answering questions like these requires inferring causality.  Realistically, if an analyst is simply not willing to have an opinion on questions of causality, their skills may be of limited value to decision-makers.
...
1.  **The Potential Outcomes Framework:** Popularized by Donald Rubin (@rubin1974), this framework is rooted in the language of experiments. It forces us to think about the "potential" state of the world under different scenarios. For each individual, we imagine two potential outcomes: the outcome if they receive a "treatment" (e.g., participation in a wellness program) and the outcome if they receive the "control" (e.g., no participation). The causal effect is the difference between these two potential outcomes. The core difficulty, which we will explore, is that we can only ever observe *one* of these outcomes for any given individual. This is what is called the *"Fundamental Problem of Causal Inference"* (@holland1986). This framework is incredibly useful for defining what a causal effect *is* with mathematical precision.  We will refer to this model as the Rubin causal model (RCM) throughout this chapter.

2.  **The Structural Causal Model (SCM) Framework:** Championed by Judea Pearl (@pearl2000), this framework uses the intuitive language of graphs to represent our causal assumptions and hypotheses. Through using *Directed Acyclic Graphs (DAGs)*, we can visualize the assumed data-generating process. By drawing nodes (variables) and arrows (causal effects), we create a causal map of the system we are studying. This map then provides us with a clear, visual guide for how to analyze our data. It tells us which variables we must control for, which we must *not* control for, and whether our causal question is even answerable with the data we have.

While historically there has been some friction between these two philosophies, they are now largely seen as two sides of the same coin. The Potential Outcomes framework provides the rigorous definition of a causal effect, while the SCM framework and its DAGs give us the tools to think about the complex system of variables in which that effect is embedded. In this chapter, we will use the Potential Outcomes framework to understand *what* we're trying to estimate and we will use DAGs to figure out *how* to estimate it from data.  For a more in-depth treatment of the Rubin causal model, see @imbens2015.  For a more in-depth treatment of the Structural Causal Model framework, see @pearl2016 or @pearl2019.  @brumback2022 and @mcelreath are also excellent resources for learning about how to apply causal inference techniques.

## Potential outcomes and typical causal inference challenges

Throughout this book we have looked at statistical effects, but we have only defined these in the context of associations between variables.  For example, in a linear regression model, we use coefficients to describe the effect of a one unit change in an input variable on the outcome variable, assuming no change in the other input variables.  A natural question that arises from this is 'if we believe an effect is causal, how do we find evidence for this?'.  For this, we need a definition of what we mean by a "causal effect." The most intuitive and mathematically rigorous way to think about this is through the Rubin Causal Model (RCM).
...
If Jane's team's sales would be \$2.1 million if she were to attend the training ($Y(1) = 2.1$) and it would be \$1.6 million if she were not to attend ($Y(0) = 1.6$), then the ITE of the training for Jane is $0.5 million. Her attendance at the training caused a \$500,000 increase in her team's sales.
...
This is the **Fundamental Problem of Causal Inference**. Causal inference is, in essence, a missing data problem. For every single person in our study, at least half of the crucial information (one of their potential outcomes) is missing. The table below illustrates this. The question marks represent the unobservable, counterfactual outcomes.
...
Now we know that we can never calculate the individual treatment effect for any single person.  But as statisticians, we could instead aim for the next best thing: estimating the average causal effect across a population of individuals.

The most common target of estimation in causal inference is the **average treatment effect (ATE)**.  This is the mean (or expected) difference in potential outcomes if *everyone* in the population were to receive the treatment versus if *everyone* were to receive the control. Formally, it is defined as:

$$
\mathrm{ATE} = E[Y(1) - Y(0)]
$$

If we estimate the ATE of our management training to be \$0.6 million, it means that, on average, the program increases average team sales by that amount. This is exactly the kind of information a decision-maker needs to evaluate the program's worth.  For example, they can compare the ATE to the cost of running the training to see if it is a good investment.  

But, again, in real life it is usually the case that not everyone is treated.  Our training program is optional and it is probably the case that not everyone will attend.   Therefore, it is more insightful for us to determine the average treatment effect for those who attend the training.  That is, we want to determine the average treatment effect for the treated, known as the ATT:
...
The simplest thing we could do is use the data we have, and calculate the difference in the average observed outcomes between the two groups: treated and not treated.  This is called the **naive comparison**.

$$
\mathrm{Naive \ Comparison} = E[Y(1) \mid T=1] - E[Y(0) \mid T=0]
$$
...
The naive comparison mixes the true causal effect of the training (ATT) with this pre-existing difference, and for this reason this bias. Our entire goal in causal inference is to find a way to eliminate this bias and isolate the true ATT.  The most effective (and usually the least practical) way to do this is through a Randomized Controlled Trial (RCT).

In an RCT, the researchers randomly assign individuals to either the treatment or the control group. Because the assignment is random, it cannot be correlated with any pre-existing characteristics of the individuals.  Over large numbers, randomization ensures that, on average, the treatment group and the control group are identical in every way *before* the treatment is administered.  Under these conditions, the selection bias term is zero and the naive comparison is an unbiased estimator of the ATT.
...
$$
0 < P(T=1 \mid X=x) < 1 \mathrm{\ and\ } 0 < P(T=0 \mid X=x) < 1 \mathrm{\ for\ all\ } x \in X
$$
...
Armed with Rubens causal model and the key assumptions above, we can now articulate the common types of problems we might encounter in attempting to infer causality between a treatment and an outcome.  All of these "boobytraps" relate to how we handle other variables outside the treatment variable $X$ and the outcome variable $Y$ in different situations.  

*   **Confounding:** As we have seen, this is the central problem. A third variable (or set of variables) $X$ is a common cause of both the treatment $T$ and the outcome $Y$. The relates to the assumption of ignorability.  To obtain an unbiased estimate of the ATT, we must identify and condition on all confounding variables.  In our management training example, if we fail to account for the managers' motivation level, which affects both their likelihood of taking the training and their team's sales, our estimate of the training's effect will be biased.

*   **Selection Bias:** While confounding is a form of selection bias, this term specifically refers to a situation where the sample is selected (or the analysis is conditioned) based on factors related to the treatment's success. This relates to the assumption of positivity.  If certain types of individuals are systematically excluded from the analysis based on their treatment status and outcome, it can bias our estimates.  For example, if we only analyze data from managers who had high sales, we might overestimate the training's effectiveness because we are ignoring those who had low sales.

*   **Mediation:** In understanding causal mechanisms, it is critical to consider if a causal effect is operating via some intermediate variables or *mediators*.  This relates to the assumption of SUTVA.  If we condition on a mediator variable that lies on the causal path from treatment to outcome, we can block part of the causal effect we are trying to estimate. For example, imagine if the training results in managers teaching new sales techniques to their teams, which then impacts sales.  If we condition our analysis on a measure of the sales techniques used by the team, we will underestimate the total causal effect of the training on sales.

The Potential Outcomes framework gives us a clear language to define the causal effects we want to estimate. The assumptions of SUTVA, Ignorability, and Positivity define the "rules of the game" for when we can legitimately estimate those effects from observational data. The challenge, however, is that the crucial assumption of Ignorability depends on identifying the correct set of confounding variables *X*. How do we do that? Simply throwing every variable we have into a regression model is not the answer and can, in fact, make things worse.


## Directed Acyclic Graphs (DAGs) as a tool for causal inference

The Potential Outcomes framework is powerful for formally defining causal effects, but it only takes us so far. It tells us we need to condition on all confounders, but it doesn't tell us how to identify them. For this, we will need to incorporate our subject-matter knowledge and our theories about how the system we are researching actually works.  We can express our causal theories using **Directed Acyclic Graphs (DAGs)**. 

DAGs are the cornerstone of the Structural Causal Model (SCM) framework. They are a visual language for expressing our assumptions about the causal relationships between variables. By representing our assumptions in a graph, we can use a clear set of rules to determine the statistical implications of those assumptions and to estimate causal effects.

### The Grammar of DAGs

A DAG consists of a two simple components:
...
*   **Edges (or Arrows):** These are directed arrows that connect two nodes. An arrow from A to B (A -> B) represents a direct causal effect. It means we are *assuming* that variable A has a direct causal influence on variable B. The arrow represents a hypothetical intervention: if we were to intervene and change the value of A, we would expect to see a change in the value of B. "Direct" here means not mediated by any other variable already in the graph.
...
Let's look at the simple example of our sales manager training program. Let's say we believe that attendance at the training program has a direct causal effect on the sales of the manager's team. Let's also say we believe that a manager's pre-existing motivation influences both their decision to sign up for the training and their team's sales.  The DAG for this causal theory would is shown in @fig-dagsimple.
...
DAG representing the causal relationships between Motivation, Training, and Sales.
:::

This simple graph encodes several causal beliefs:
1.  `Motivation` causes `Training` (more motivated managers are more likely to sign up).
2.  `Motivation` causes `Sales` (more motivated managers lead their teams to higher sales figures).
3.  `Training` causes `Sales` (this is the effect we want to estimate).
4.   There are no other common causes of any two variables in the graph.
5.  `Sales` does not cause `Training` (the outcome doesn't affect the treatment). 
...
chain_plot <- ggdag(chain_dag, stylized = TRUE) +
  geom_dag_node(color = "lightblue", size = 15) +
  geom_dag_text(color = "black", size = 4) +
  theme_dag() +
  ggtitle("Chain (Mediation)") +
  theme(plot.title = element_text(hjust = 0.5)) 

fork_plot <- ggdag(fork_dag, stylized = TRUE) +
  geom_dag_node(color = "lightblue", size = 15) +
  geom_dag_text(color = "black", size = 4) +
  theme_dag() +
...
collider_plot <- ggdag(collider_dag, stylized = TRUE) +
  geom_dag_node(color = "lightblue", size = 15) +
  geom_dag_text(color = "black", size = 4) +
  theme_dag() +
...
The three fundamental structures in DAGs: Chain (Mediation), Fork (Confounding), and Collider (Selection Bias). In each diagram, A and B are the variables for which we are estimating the effect.  In the chain, M is the mediator. In the fork, C is the common cause (confounder). In the collider, D is the collider and E is a descendent collider.
:::



* Mediation is represented by a 'chain' structure of the form `A -> M -> B`.  This means A causes M, and M causes B.  M is a mediator variable. In a chain, there is a clear causal path from A to B. Therefore, A and B will be statistically associated, but if we condition on the mediator M, we *block* the flow of association between A and B.  For example, if we believe `Motivation -> Job Satisfaction -> Job Performance`, then if we try to estimate the effect of `Motivation` on `Job Performance` we should not include `Job Satisfaction` in our statistical model.  If we do, we will have a biased estimate.

* Confounding is represented by a 'fork' structure of the form `A <- C -> B`.  This means C is a common cause of both A and B. In a fork, there is no causal path from A to B. However, because they share a common cause, information flows from C to A and from C to B, creating a spurious (non-causal) association between A and B. If we were to naively correlate A and B, we would find an association, not because A causes B, but because C causes both. The non-causal path from A to B is called a 'backdoor' path, because A has an edge going into it rather than coming out of it. To block this backdoor path, we must condition on the common cause, C. For example, if we believe `Training <- Motivation -> Sales`, then to estimate the causal effect of `Training` on `Sales`, we must include `Motivation` in our model.  If we leave it out, we will have a biased estimate.

* Selection bias is represented by a 'collider' structure of the form `A -> D <- B`.  This means A and B are both independent causes of a third variable, D. The arrows "collide" at D. In a collider structure, if A and B are independent to begin with, there is no open path between them. The collider D *naturally blocks* the flow of association. However, if we condition on the collider, D, we *open the path* and create a spurious association between A and B where none existed before. For example, if we believe `Knowledge -> Being Hired <- Interpersonal Skills`, then in the general population, `Knowledge` and `Interpersonal Skills` are uncorrelated. But if we look only at only those who were hired, then within that selected group, `Knowledge` and `Interpersonal Skills` can become negatively correlated. Conditioning on the common effect (the collider) induces an association.  Moreover, if a variable is a descendant of a collider (i.e., it is on a chain emanating from the collider, such as our variable `E` in @fig-dagstructures), conditioning on that descendant also opens the path.

This leaves us with three critical rules for how associations (paths) flow through a DAG.
...
The most important application of these rules is in finding a set of variables to control for to estimate a causal effect. Let's say we want to estimate the causal effect of a treatment, T, on an outcome, Y (the causal path from T to Y).

The statistical association we measure between T and Y in our data is a mixture of the true causal effect (the causal path from T to Y) and any spurious associations from non-causal paths. Our goal is to isolate the causal path. To do this, we need to block all the open non-causal paths between T and Y, without blocking the causal path itself.

The most common type of non-causal path is the **backdoor path**. A backdoor path is any path that starts with an arrow pointing *into* T. For example, in our motivation fork (`Training` <- `Motivation` -> `Sales`), the path from `Training` to `Motivation` to `Sales` is a backdoor path. It creates a spurious association that we must block.
...
If we can find a set of variables *Z* that satisfies the backdoor criterion, then we can estimate the causal effect of T on Y by conditioning on Z. In practice, this means including Z as input variables in a regression model.  Let's look at a specific example in order to see this criterion in action.
...
*   The test, the interview and the presentation all cause the hiring decision.
*   The presentation causes the interview score because the interview is conducted by a member of the panel.
*   The academic ability of the graduate causes their test score, interview score and presentation score.
...
* `Pres`: The candidate's average rating from a presentation to a panel, on an scale from 1 (Low) to 5 (High).
* `GPA`: The candidate's undergraduate Grade Point Average (GPA), on a scale from 1.0 to 5.0.
...
Before we define a causal model to work with, let's look at what a 'naive' analysis would look like.  We can run a simple Bayesian logistic regression predicting `Hire` from `Test` alone.
...
We can now take a look at our posterior odds ratio for `Test`, as in @fig-naiveposterior.
...
mcmc_areas(
  exp(as.matrix(naive_model)),
  pars = "b_Test",
  prob = 0.95
) +
  theme_minimal()
```
Posterior distribution of the odds ratio for `Test` from a naive Bayesian logistic regression predicting `Hire` from `Test` alone.
:::
...
This naive model suggests a very high probability (`r round(prob_positive_naive*100, 1)`%) that the test has a positive effect on the odds of being hired.  However, we have no idea if this is a causal effect.  There may be confounding variables biasing this estimate.  To determine what variables to control for, we need to define our causal model using a DAG.
...
@fig-selection-ci shows the posterior distribution of the GPA coefficient.  The 95% credible interval includes zero, suggesting that after controlling for `Int`, `Pres` and `Test`, `GPA` there is insufficient evidence for a significant effect on the hiring decision. This supports the conditional independence implied by our DAG.
...
We can visualize our causal effect estimate for `Test` on the odds of being hired, as in @fig-selection-causal.
...
mcmc_areas(
  exp(as.matrix(model_causal)),
  pars = c("b_Test"),
  prob = 0.95
) + 
  theme_minimal()
```
Posterior distribution of the causal effect of Test on Hire, expressed as an odds ratio.
:::

We can also view the 95% credible interval for the odds ratio of `Test` on `Hire`.

```{r}
# get 95% credible interval for the odds ratio of Test
(ci <- posterior_summary(exp(as.matrix(model_causal)), probs = c(0.025, 0.975))["b_Test", c("Q2.5", "Q97.5")])
```

This suggests that, under the assumptions of our causal model, we cannot rule out that the test has no causal effect on the odds of being hired.  We can determine the probability that the effect of the test score is positive by calculating the proportion of posterior samples where the coefficient for `Test` is greater than zero.
...
This indicates that there is a `r round(prob_positive*100, 1)`% probability that the test score has a positive causal effect on the odds of being hired.  This is a big difference from our naive estimation, suggesting that controlling for our confounder `GPA` has had a major impact on our estimate.

## Learning exercises

### Discussion questions

1.  What are the different types of treatment effects defined in the Rubin causal model? How are they defined mathematically?

2.  What is 'bias' in the context of the Potential Outcomes framework?  Give a common reason why the naive estimate of a treatment effect might be biased.
...
For our data exercises in this chapter, we will return to the `sociological_data` dataset in @sec-lin-reg-ols-exercises.  Take a random sample of about 20% of the observations and assume that this is the only data you have available.  For this exercise, our aim will be to determine the causal effect of `education_months` on `annual_income_ppp`.  We will assume that we have the following causal model in mind:
...
7.  Finally, estimate the causal effect of `education_months` on `annual_income_ppp` using a Bayesian regression model that controls for the variables in your adjustment set.  
