...
The `selection` data set contains data related to the hiring process for 200 candidates in a graduate hiring program. The data contains the following variables:
...
We have a hypothesized causal model for how these variables relate to each other, as shown earlier in @fig-dagcomplex. We want to estimate the causal effect of `Test` on `Hire`. According to our earlier backdoor analysis, we should be able to do this by controlling for either `GPA` alone, or both `Int` and `Pres`.
...
This suggests that, under the assumptions of our causal model, there is a 95% probability that a one-point increase in the test score increases the odds of being hired by between `round((ci["Q2.5"] -1)*100)`% and `round((ci["Q97.5"] -1)*100)`%.
...
For our data exercises in this chapter, we will return to the `sociological_data` dataset in @sec-lin-reg-ols-exercises.  For this exercise, our aim will be to determine the causal effect of `education_months` on `annual_income_ppp`.  We will assume that we have the following causal model in mind:
...
1.  Using the `dagitty` or `ggdag` package in R, specify the DAG representing the causal model described above.

2.  Visualize the DAG using `ggdag`. Make sure to label the nodes clearly.
...
8.  Visualize the posterior distribution of the causal effect estimate and interpret the results.  Give the 95% credible interval for the causal effect estimate.
