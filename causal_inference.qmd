...
mcmc_areas(
  ...
  theme_minimal()
```
Posterior distribution of the coefficient for `Test` from a naive Bayesian logistic regression predicting `Hire` from `Test` alone.
:::
...
We can view thw causal graph using the `view_graph()` method as in @fig-py-causal-graph:
