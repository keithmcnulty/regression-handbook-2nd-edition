...
`r if (knitr::is_latex_output()) '\\index{statistical power}'`*Statistical power* refers to the fourth situation and is the *probability that $H_0$ is rejected given that $H_1$ is true*.  Statistical power depends *at a minimum* on three criteria:
