...
### Intuition for proportional odds logistic regression {#sec-ord-intuit}
...
tau_label <- TeX("$y' = \\tau_3$", output = "character")
...
p1 <- ggplot() +
  ...
  annotate("text", x = 0.5, y = 3.2, label = tau_label, color = "red", parse = TRUE) +
  theme_minimal() +
...
Proportional odds model illustration for a 5-point job performance scale outcome greater than 3 on a single input variable.  Each cutoff point in the latent continuous outcome variable gives rise to a binomial logistic function.
:::
...
### Walkthrough example {#sec-ord-walkthrough}
...
\index{data sets!soccer@\texttt{soccer}}Let's download the `soccer` data set and take a quick look at it.
...
Since our only values for $y_i$ are 1, 2 and 3, similar to our derivations in @sec-mod-prob, we conclude that $P(y_i > 1) = 1 - P(y_i = 1)$, which calculates to
...
\index{R!packages!MASS@\texttt{MASS}}The `MASS` package provides a function `polr()` for running a proportional odds logistic regression model on a data set in a similar way to our previous models.  The key (and obvious) requirement is that the outcome is an ordered factor.  Since we did our conversions in @sec-ord-walkthrough we are ready to run this model. We will start by running it on all input variables and let the `polr()` function handle our dummy variables automatically.
...
Recall from @sec-mod-lin-reg that our proportional odds model generates multiple stratified binomial models, each of which has following form:
...
3D visualization of a simple proportional odds model for `discipline` fitted against `n_yellow_25` and `n_red_25` in the `soccer` data set.  Blue represents the probability of no discipline from the referee.  Yellow and red represent the probability of a yellow card and a red card, respectively.
:::
...
\index{Pseudo-R-squared!in ordinal logistic regression}Similar to binomial and multinomial models, pseudo-$R^2$ methods are available for assessing model fit, and AIC can be used to assess model parsimony.  Note that `DescTools::PseudoR2()` also offers AIC.
...
\index{proportional odds logistic regression!goodness-of-fit}There are numerous tests of goodness-of-fit that can apply to ordinal logistic regression models, and this area is the subject of considerable recent research. \index{R!packages!generalhoslem@\texttt{generalhoslem}}The `generalhoslem` package in R contains routes to four possible tests, with two of them particularly recommended for ordinal models.  Each work in a similar way to the Hosmer-Lemeshow test discussed in @sec-logistic-gof, by dividing the sample into groups and comparing the observed versus the fitted outcomes using a chi-square test.  Since the null hypothesis is a good model fit, low p-values indicate potential problems with the model.   We run these tests below for reference. For more information, see @fagerland-hosmer, and for a really intensive treatment of ordinal data modeling @ordinal is recommended. 


```{r}
# lipsitz test 
lipsitz <- generalhoslem::lipsitz.test(model)

# view statistic and p-value
c(lipsitz$statistic, pvalue = lipsitz$p.value)
```
...
pr_test <- generalhoslem::pulkrob.chisq(model, catvars = cats)

# view statistic and p-value
c(pr_test$statistic, pvalue = pr_test$p.value)
```
...
prop_odds_logit = OrderedModel.from_formula(
  """
  discipline ~ 1 + n_yellow_25 + n_red_25 + position 
  + result + country + level
  """, 
  soccer, 
  distr='logit'
...
Load the `managers` data set via the `peopleanalyticsdata` package or download it from the internet^[https://peopleanalytics-regression-book.org/data/managers.csv].  It is a set of information of 571 managers in a sales organization and consists of the following fields:
...
\index{ordinal logistic regression|see {proportional odds logistic regression}}
\index{regression!ordinal|see {proportional odds logistic regression}}
