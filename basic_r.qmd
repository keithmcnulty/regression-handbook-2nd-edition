# The Basics of the R Programming Language {#sec-basic-r}
...
\index{R!installation and use}Just like most programming languages, R itself is an interpreter which receives input and returns output.  It is not very easy to use without an IDE.  An IDE is an *Integrated Development Environment*, which is a convenient user interface allowing an R programmer to do all their main tasks including writing and running R code, saving files, viewing data and plots, integrating code into documents and many other things.  By far the most popular IDE for R is RStudio.  An example of what the RStudio IDE looks like can be seen in @fig-rstudio-ide. 
...
knitr::include_graphics("www/R/rstudio_ide_screenshot.png")
```
The RStudio IDE.
:::
...
2.  Download the latest version of the RStudio Desktop IDE from `https://posit.co/download/rstudio-desktop/`.  Consult the user guide at `https://docs.posit.co/ide/user/` to help orient yourself to the features of RStudio.  \ 
...
\index{R!data}As you start to do tasks involving data in R, you will generally want to store the things you create so that you can refer to them later.  Simply calculating something does not store it in R.  For example, a simple calculation like this can be performed easily:
...
\index{R!data types}All data in R has an associated type, to reflect the wide range of data that R is able to work with.  The `typeof()` function can be used to see the type of a single scalar value. Let's look at the most common scalar data types.
...
\index{R!vectors}**Vectors** are one-dimensional structures containing data of the same type and are notated by using `c()`. The type of the vector can also be viewed using the `typeof()` function, but the `str()` function can be used to display both the contents of the vector and its type.
...
\index{R!categorical factors}Categorical data---which takes only a finite number of possible values---can be stored as a factor vector to make it easier to perform grouping and manipulation.
...
\index{R!ordered factors}If needed, the factors can be given order.
...
\index{R!type coercion}If you try to mix data types inside a vector, it will usually result in *type coercion*, where one or more of the types are forced into a different type to ensure homogeneity. Often this means the vector will become a character vector.
...
\index{R!matrices}**Matrices** are two-dimensional data structures of the same type and are built from a vector by defining the number of rows and columns.  Data is read into the matrix down the columns, starting left and moving right.  Matrices are rarely used for non-numeric data types.
...
\index{R!lists}**Lists** are one-dimensional data structures that can take data of any type.
...
\index{R!dataframes}**Dataframes** are the most used data structure in R; they are effectively a named list of vectors of the same length, with each vector as a column.  As such, a dataframe is very similar in nature to a typical database table or spreadsheet.  
...
We can view a specific column by using `$`, and we can use square brackets to view a specific entry.   For example if we wanted to see the sixth entry of the `sales` column:
...
\index{R!subsetting dataframes}Dataframes can be subsetted to contain only rows that satisfy specific conditions.
...
\index{R!combining dataframes}Two dataframes with the same column names can be combined by their rows.
...
```
Results of a search for the `head()` function in the RStudio Help browser.
...
df_report <- function(df) {
  paste("This dataframe contains", nrow(df), "rows and", 
        ncol(df), "columns, with", sum(is.na(df)), "NA entries.")
}
```
...
\index{R!pipe operator}Even in the most elementary briefing about R, it is very difficult to ignore the pipe operator.  The pipe operator makes code more natural to read and write and reduces the typical computing problem of many nested operations inside parentheses.  
...
\index{R!errors}*Errors* are serious problems which usually result in the halting of your code and a failure to return your requested output.  They usually come with an indication of the source of the error, and these can sometimes be easy to understand and sometimes frustratingly vague and abstract.  For example, an easy-to-understand error is:
...
\index{R!warnings}*Warnings* are less serious and usually alert you to something that you might be overlooking and which could indicate a problem with the output.  In many cases you can ignore warnings, but sometimes they are an important reminder to go back and edit your code.  For example, you may run a model which doesn't converge, and while this does not stop R from returning results, it is also very useful for you to know that it didn't converge.  

\index{R!messages}*Messages* are pieces of information that may or may not be useful to you at a particular point in time.  Sometimes you will receive messages when you load a package from your library.  Sometimes messages will keep you up to date on the progress of a process that is taking a long time to execute.   
...
\index{R!scatter plots}The simplest plot function in base R is `plot()`.  This performs basic X-Y plotting.  As an example, this code will generate a scatter plot of `customer_rate` against `sales` in the `salespeople` data set, with the results displayed in @fig-basicscatter.  Note the use of the arguments `main`, `xlab` and `ylab` for customizing the axis labels and title for the plot.
...
```
Simple scatterplot of `customer_rate` against `sales` in the `salespeople` data set.
:::

\index{R!histograms}Histograms of data can be generated using the `hist()` function.  This command will generate a histogram of `performance` as displayed in @fig-basichist.  Note the use of `breaks` to customize how the bars appear.
...
Simple histogram of `performance` in the `salespeople` data set.
:::

\index{R!box and whisker plots}Box and whisker plots are excellent ways to see the distribution of a variable, and can be grouped against another variable to see bivariate patterns.  For example, this command will show a box and whisker plot of `sales` grouped against `performance`, with the output shown in @fig-basicbox.  Note the use of the `formula` and `data` notation here to define the variable we are interested in and how we want it grouped.  We will study this formula notation in greater depth later in this book. 
...
Simple box plot of `sales` grouped against `performance` in the `salespeople` data set.
:::
...
\index{R!packages!ggplot2@\texttt{ggplot2}}By far the most commonly used specialist plotting and graphing package in R is `ggplot2`.  `ggplot2` allows the flexible construction of a very wide range of charts and graphs, but uses a very specific command grammar which can take some getting used to.  However, once learned, `ggplot2` can be an extremely powerful tool.  Many of the illustratory figures used in this book are developed using `ggplot2` and while the code for these figures is sometimes not included for the sake of brevity, you can always find it in the source code of this book on Github^[https://github.com/keithmcnulty/peopleanalytics-regression-book].  A great learning resource for `ggplot2` is @ggplot2.

\index{R!packages!ggplot2@\texttt{plotly}}The `plotly` package allows the use of the `plotly` graphing library in R.  This is an excellent package for interactive graphing and is used for 3D illustrations in this book.  Output can be rendered in HTML---allowing the user to play with and explore the graphs interactively---or can be saved as static 2D images.

\index{R!packages!ggplot2@\texttt{GGally}}`GGally` is a package that extends `ggplot2` to allow easy combination of charts and graphs.  This is particularly valuable for quicker exploratory data analysis.  One of its most popular functions is `ggpairs()`, which produces a pairplot.  \index{R!pairplot}A pairplot is a visualization of all univariate and bivariate patterns in a data set, with univariate distributions in the diagonal and bivariate relationships or correlations displayed in the off-diagonal.  @fig-basicpair is an example of a pairplot for the `salespeople` data set, which we will explore further in @sec-bin-log-reg.
...
Pairplot of the `salespeople` data set.
:::

## Documenting your work using Quarto
...
\index{Quarto}Quarto is a package which allows you to create integrated documents containing both formatted text and executed code.  It is, in my opinion, one of the best resources available currently for this purpose. This entire book has been created using Quarto. You can start a Quarto document in RStudio by opening a new Quarto document file, which will have the suffix `.qmd`.
...
You can set options for your code chunks inside a code block by starting your line with `#|`.  For example, if you want the results of your code to be displayed, but without the code itself being displayed, you can start your code block with the line 
...
13.  What is Quarto, and why is it useful to someone performing analysis using programming languages?
...
5.  \index{data sets!ugtests@\texttt{ugtests}}Load the `ugtests` data set via the `peopleanalyticsdata` package or download it from the internet^[https://peopleanalytics-regression-book.org/data/ugtests.csv].  Calculate the dimensions of `ugtests` and view the first three rows only. 
...
14.  Render all of your answers to these exercises into a Quarto document.  Create one version that displays your code and answers, and another that just displays the answers.
