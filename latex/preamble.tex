...
\newglossaryentry{binomial distribution}
{
    name={binomial distribution},
    description={A discrete probability distribution modeling the number of successes in a fixed number of independent Bernoulli binary trials (yes/no), where each trial has a the same constant success probability}
}

\newglossaryentry{Poisson distribution}
{
    name={Poisson distribution},
    description={A discrete probability distribution that models the number of times an event occurs within a specific interval of time, space, or volume. It is defined by a single parameter \ensuremath{\lambda}, which represents the average number of occurrences (mean) per interval}
}

\newglossaryentry{Poisson regression}
{
    name={Poisson regression},
    description={A generalized linear model used to explain or predict count data (non-negative integers) or rates, such as the number of events occurring within a specific time, area, or volume}
}

\newglossaryentry{overdispersion}
{
    name={overdispersion},
    description={Occurs in statistical modeling when observed data has higher variance than the theoretical model predicts}
}

\newglossaryentry{underdispersion}
{
    name={underdispersion},
    description={Occurs in statistical modeling when observed data has lower variance than the theoretical model predicts}
}

\newglossaryentry{quasi-Poisson regression}
{
    name={quasi-Poisson regression},
    description={A generalization of Poisson regression for modeling count data that exhibits overdispersion, violating standard Poisson assumptions}
}

\newglossaryentry{negative binomial regression}
{
    name={negative binomial regression},
    description={A generalized linear model used to analyze count data with overdispersion, making it a robust alternative to Poisson regression}
}

\newglossaryentry{zero-inflation}
{
    name={zero-inflation},
    description={Describes a situation where data contains more zeros than would be expected based on count distributuons like Poisson or negative binomial distributions, and where some zeros are believed to occur due to structural
    reasons separate from the expected count distribution}
}

\newglossaryentry{Bayesian inference}
{
    name={Bayesian inference},
    description={A statistical method that uses Bayes' Theorem to update the probability of a hypothesis or parameter value as more evidence or information becomes available, starting from an initial belief}
}

\newglossaryentry{prior}
{
    name={prior},
    description={In Bayesian inference, the prior is the initial belief about the probability of a hypothesis or parameter value before evidence (data) is made available, usually in the form of a statistical distribution}
}

\newglossaryentry{posterior}
{
    name={posterior},
    description={In Bayesian inference, the posterior is the updated belief about the probability of a hypothesis or parameter value after evidence (data) is taken into account, usually in the form of a statistical distribution}
}

\newglossaryentry{beta distribution}
{
    name={beta distribution},
    description={A family of continuous probability distributions defined on the interval [0,1], characterized by two shape parameters and often used to model proportions or probabilities}
}

\newglossaryentry{credible interval}
{
    name={credible interval},
    description={A Bayesian statistics concept representing a range of values that an unknown parameter falls within with a specific probability (e.g., 95\%) based on observed data and prior beliefs}
}

\newglossaryentry{Bayes' factor}
{
    name={Bayes' factor},
    description={A Bayesian model comparison tool that quantifies the evidence provided by data for one hypothesis over another, calculated as the ratio of their marginal likelihoods}
}

\newglossaryentry{Region of Practical Equivalence (ROPE)}
{
    name={Region of Practical Equivalence (ROPE)},
    description={A Bayesian statistical method defining a range of parameter values—--often around a null value—--that are considered "practically equivalent" to zero or a baseline}
}

\newglossaryentry{causal inference}
{
    name={causal inference},
    description={The process of determining the independent, actual effect of a phenomenon, moving beyond mere correlation to understand how actions, treatments or interventions cause outcomes}
}

\newglossaryentry{confounding}
{
    name={confounding},
    description={A distortion in measuring the relationship between an exposure (independent variable) and an outcome (dependent variable) caused by a third, external variable (the confounder) that influences both}
}

\newglossaryentry{selection bias}
{
    name={selection bias},
    description={A systematic error occurring when study participants are chosen in a non-random way, leading to a sample that is not representative of the target population.  This leads to distorted inferences from data}
}

\newglossaryentry{mediation}
{
    name={mediation},
    description={A situation where an independent variable affects a dependent variable through an intervening mediator variable---an important consideration in causal inference}
}

\newglossaryentry{directed acyclic graph (DAG)}
{
    name={directed acyclic graph (DAG)},
    description={A visual framework for describing beliefs about the causal effects of variables on other veriables within a system.  A key tool in causal inference}
}
