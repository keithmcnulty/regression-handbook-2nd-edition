...
\index{Bayes' Theorem}The critical theorem from probability theory that underpins this approach is *Bayes' Theorem*, named after the Reverend Thomas Bayes, an 18th century statistician and theologian.  As we will see, Bayes' Theorem provides us with a mathematical way to combine our prior beliefs about a parameter with the evidence provided by our data to arrive at an updated *posterior* belief about the parameter.  \index{Bayesian inference}This mechanism is the essence of *Bayesian inference*.  

Bayesian inference has become increasingly popular in recent years.  There are a number of philosophical and epistemological reasons behind the recent growth in its use and application, but a key practical enabler for this growth has been the rapid development in computational technology in the early 21st century. Techniques used in Bayesian inference require simulation of data, which can demand substantially more computational resources compared to classical approaches. Only in the past two decades has widespread computational power become available that has made it more feasible to apply Bayesian methods to large data sets.  Bayesian approaches are now widely used in fields such as epidemiology, econometrics, ecology, anthropology and the social sciences, to name but a small few.  All of the methods that we have learned so far using classical statistics have Bayesian counterparts.  
...
\index{Bayesian inference!posterior probability (distribution)}
\index{Bayesian inference!likelihood function}
\index{Bayesian inference!prior probability (distribution)}
...
Informative Beta(8, 2) prior distribution of pass rate ($\theta$) before observing any data.
:::
...
# (divide by total area to ensure it integates to 1)
posterior_distribution <- unnormalized_posterior / 
...
posterior_df <- data.frame(
  theta = theta_values, 
  density = posterior_distribution
)
...
ggplot(combined_df, aes(x = theta, y = density, color = Type, fill = Type)) +
  ...
  scale_color_manual(values = c("magenta", "cyan", "blue")) +
  scale_fill_manual(values = c("magenta", "cyan", "blue")) +
  labs(x = expression(theta),
       y = "Probability",
       fill = "Type") +
  theme_minimal() +
...
new_likelihood_function <- dbinom(
  new_passes, 
  size = new_passes + new_fails, 
  prob = theta_values
)

# calculate the unnormalized new posterior distribution
unnormalized_new_posterior <- new_prior_distribution * 
  new_likelihood_function

# normalize the new posterior distribution
new_posterior_distribution <- unnormalized_new_posterior / 
  sum(unnormalized_new_posterior)

# plot the new posterior distribution
new_posterior_df <- data.frame(
  theta = theta_values, 
  density = new_posterior_distribution
)
...
(median_posterior <- theta_values[
  which(cumsum(new_posterior_distribution) >= 0.5)[1]
])

# calculate the mode of the posterior distribution
(mode_posterior <- theta_values[
  which.max(new_posterior_distribution)
])
```

Additionally, given that we have determined a specific distribution for the pass rate $\theta$, we can calculate \index{credible interval}*credible intervals* to express the uncertainty around our estimate.  A credible interval is an interval within which the parameter is believed to lie with a certain probability, based on the posterior distribution.  For example, a 95% credible interval for the pass rate $\theta$ would indicate that there is a 95% probability that the true pass rate lies within this interval, given the observed data.  We can calculate the 95% credible interval for our updated posterior distribution using the `quantile()` function in R:


```{r}
# calculate the 95% credible interval for 
# the updated posterior distribution
interval_boundaries <- quantile(
...
A common rule of thumb^[@jeffreys] for interpreting Bayes Factors which favor the Alternative Hypothesis is:
...
To implement Bayesian hypothesis tests in R, we will utilize the \index{R!packages!BayesFactor@\texttt{BayesFactor}}`BayesFactor` package. This package allows us to perform statistical hypothesis tests within a Bayesian framework without needing to write complex custom simulations from scratch.  These tests use standard minimally informed priors for the specific tests being used.  To illustrate these Bayesian tests, we will use the same examples of hypothesis tests on our \index{data sets!salespeople@\texttt{salespeople}}`salespeople` data set as we used in @sec-found-stats for classical statistics, but now using smaller subsets of this data. Let's download that set first.
...
### Bayesian $t$-test for difference in means between two independent groups {#sec-bayes-ttest}
...
# plot posterior distributions of sales for 
# low and high performing salespeople
...
We can also calculate summary statistics and credible intervals for the posterior distribution of the sales difference between high and low performing salespeople, using the convenient `describe_posterior()` function from the \index{R!packages!bayestestR@\texttt{bayestestR}}`bayestestR` package.
...
### Bayesian test for non-zero correlation {#sec-bayes-corr}
...
correlationBF(
  sample_salespeople$sales, 
  sample_salespeople$customer_rate
)
```
...
```{r}
#| eval: false
# run Bayesian correlation test and extract posterior samples 
# over 10000 simulations
set.seed(42)
bf_corr_result <- correlationBF(
  sample_salespeople$sales, 
  sample_salespeople$customer_rate
)

posterior_corr_samples <- posterior(
  bf_corr_result, 
  iterations = 10000
)
...
# create a halfeye plot of posterior distribution 
...
```{r}
#| eval: false
# run Bayesian contingency table test and extract 
# posterior samples over 10000 simulations
set.seed(42)
bf_contingency_result <- contingencyTableBF(
  contingency, 
  sampleType = "jointMulti"
)

posterior_contingency_samples <- posterior(
  bf_contingency_result, 
  iterations = 10000) |> 
  as.data.frame()

# calculate relative promotion rates within each performance category
for (i in 1:4) {
  posterior_contingency_samples[, paste0("pi[", i, "]")] <- 
    posterior_contingency_samples[, paste0("pi[2,", i, "]")] / 
    (posterior_contingency_samples[, paste0("pi[1,", i, "]")] + 
       posterior_contingency_samples[, paste0("pi[2,", i, "]")])
}
...
posterior_contingency_df <- data.frame(
  promotion_rate = c(
    as.vector(posterior_contingency_samples[, "pi[1]"]),
  ...
  performance_rating = rep(
    1:4, 
    each = nrow(posterior_contingency_samples))
)


# plot posterior distributions of promotion rates using halfeye plot
ggplot(
  posterior_contingency_df, 
  aes(x = promotion_rate, y = as.factor(performance_rating))
) +
  stat_halfeye(
    fill = "lightblue",
    color = "blue"
...
### Bayesian hypothesis testing using Python

\index{hypothesis testing!Bayesian!using Python}\index{Python!packages!pingouin@\texttt{pingouin}}The `pingouin` package in Python offers very simple functions for Bayes Factor estimations against a null hypothesis.  These functions accept the relevant statistics (eg a $t$-statistic for a $t$-test as well as the size of both groups) and returns the Bayes Factor.  For example, to calculate our $t$-test Bayes Factor as in @sec-bayes-ttest, we would first determine the $t$-statistic for the difference in means of the two groups as follows^[Note that setting the same random seed in both R and Python does not produce the same randomization process, so you can expect analyses to differ between the two languages when randomization is required.]:
...
And similarly, to calculate the Bayes Factor for the correlation test as in @sec-bayes-corr, we would do the following:
...
6.  How does the interpretation of a *credible* interval differ from a *confidence* interval?
7.  Why does Bayesian inference allow us to make direct statements about the probability of our parameter taking specific values?
...
\index{data sets!ugtests@\texttt{ugtests}}Load the `ugtests` data set which we used in @sec-linear-reg-ols via the `peopleanalyticsdata` package or download it from the internet^[https://peopleanalytics-regression-book.org/data/ugtests.csv]. Remind yourself of what this data set contains.
...
7.  Visualize the posterior distribution of the difference in Year 2 score means between the two groups using a halfeye plot.
8.  Calculate summary statistics and a 95% credible interval for the posterior distribution of the difference in Year 2 scores between the two groups. 
