# Bayesian Inference - A Modern Alternative to Classical Statistical Methods
...
In this chapter we will explore the fundamental mechanics of how Bayesian inference works via Bayes' Theorem, we will introduce how hypothesis testing can be done within the Bayesian framework, and we will introduce some important concepts that we can take forward into subsequent chapters.  
...
Non-informative Beta (1, 1) (uniform) prior on distribution of pass rate ($\theta$) before observing any data.
...
# simulate the prior distribution using a Beta distribution with alpha = 8 and beta = 2
prior_distribution <- dbeta(theta_values, shape1 = 8, shape2 = 2)
```

This has provided us with the probability density function (PDF) across our 1000 values of $\theta$^[Note that this is a continuous PDF and therefore does not plot the specific probabilities for $\theta$.  One way to interpret the density function is that if $\theta_1$ has a density of 1 and $\theta_2$ has a density of 2, then $\theta_2$ is twice as likely to occur compared to $\theta_1$.  If we wished, we could normalize this simulated density to provide specific probabilities by dividing each our 1000 values by their total sum, but this would make no difference to the output of Bayes' Theorem,  If you are unconvinced by this, try rerunning everything with a normalized PDF as a prior.].  Let's now plot our simulated prior distribution to visualize our initial beliefs about the pass rate before observing any data, as in @fig-pass-prior.
...
combined_df <- data.frame(
  ...
  density = c(prior_distribution/sum(prior_distribution), likelihood_function / sum(likelihood_function), posterior_distribution),
  Type = rep(c("Prior Distribution", "Likelihood Function", "Posterior Distribution"), each = length(theta_values))
)
...
To implement Bayesian hypothesis tests in R, we will utilize the `BayesFactor` package. This package allows us to perform statistical hypothesis tests within a Bayesian framework without needing to write complex custom simulations from scratch.  To illustrate these tests, we will use the same examples of hypothesis tests on our `salespeople` data set as we used in @sec-found-stats for classical statistics, but now using smaller subsets of this data and a Bayesian approach. Let's download that set first.
...
We see that the Bayes Factor $BF_{10}$ is over just over 3, indicating that the data would approximately triple any prior belief we have about the likelihood of the alternative hypothesis.  This is moderate evidence in favor of the data supporting the Alternative Hypothesis ($H_1$) that there is a difference in mean sales between high and low performing salespeople.  

Note that because of the Bayes Factor, we did not even have to look at the posterior distributions of the difference in means to make this conclusion.  However, we can still plot these posterior distributions to visualize the difference in means between the two groups, as in @fig-bayes-ttest.
...
```
Therefore we can say that there is a `r round(describe_posterior(posterior_corr_samples)$pd[1], 2)` probability that the correlation between sales and average customer rating is greater than zero given the data we observe.  We see that the 95% credible interval has a very small overlap with the ROPE around zero, indicating that the correlation is likely to be non-zero in practical terms.
...
Now we can apply the hypothesis, but before we do so we need to define the sampling method in order to inform the simulation.  For this we have four choices:
...
1.  The academic administrator informs you a score of 40 represents a Pass.  It is their belief that 'somewhere around 7 in 10' students will pass the Year 1 examination.  Simulate an informative prior distribution for the pass rate based on this belief.  Plot the prior distribution.

2.  Using your first set of observed data, simulate the likelihood function for the pass rate.  

3.  Use your work from questions 1 and 2 to calculate the posterior distribution for the Year 1 pass rate.  Plot the prior, likelihood and posterior together on a standardized plot to compare them.

4.  Using the posterior distribution from question 3 as your new prior, and using your second set of observed data, calculate the updated posterior distribution for the Year 1 pass rate.  Plot the updated posterior distribution.
...
For questions 6 to 10, take a random sample of 50 rows for the full `ugtests` data set and treat this as your observed data.  

6.  Test the hypothesis that there is a difference in Year 2 score between top quartile students and bottom quartile students from Year 1.  Use a Bayesian t-test to perform this test.  Report and interpret the Bayes Factor.  
...
9.  Test the hypothesis that there is a non-zero correlation between Year 2 and Final Year examination scores using a Bayesian correlation test.  Report and interpret the Bayes Factor.
