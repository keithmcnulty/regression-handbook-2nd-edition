...
The `pingouin` package in Python offers very simple functions for Bayes Factor estimations against a null hypothesis.  These functions accept the relevant statistics (eg a $t$-statistic for a $t$-test as well as the size of both groups) and returns the Bayes Factor.  For example, to calculate our $t$-test Bayes Factor as in @sec-bayes-ttest, we would first determine the $t$-statistic for the difference in means of the two groups as follows:
